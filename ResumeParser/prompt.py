# khung json (định nghĩa thuộc tính các trường của json)
import json
...
def post_parse_cv(output): # lọc ra json từ kết quả trả về của AI
...
def prompt_to_write_description(resp, title, company, description): #viết lại phần mô tả công việc của CV gốc
    prompt = f"""
...
def post_write_description(output): return output[12:].strip() # bỏ đi 12 ký tự đầu sau khi sinh ra output của description (bỏ từ description)

def prompt_to_rewrite_task(resp, title, company, description): #chỉnh lại phần responsibilities
    prompt = f"""
    You are a career consultant.
    You are improving the work experience section in your customer's resume.
...
def post_rewrite_task(output): return output[17:].strip() #bỏ từ responsibilities
    
def prompt_to_add_skills(skills, resume_json, domain='IT'): #cải thiện skills
    format = """
    [
      {"skill_name": "<extra skill 1: str>"},
      {"skill_name": "<extra skill 2: str>"},
    ]
    """
